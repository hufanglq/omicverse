...
def _highly_variable_genes_seurat_v3(  # noqa: PLR0912, PLR0915
    ...
    **variances_norm**
    ...
    if inplace:
        ...
        print(f"     {Colors.CYAN}• 'highly_variable': {Colors.BOLD}Boolean vector{Colors.ENDC}{Colors.CYAN} (adata.var){Colors.ENDC}")
        print(f"     {Colors.CYAN}• 'highly_variable_rank': {Colors.BOLD}Float vector{Colors.ENDC}{Colors.CYAN} (adata.var){Colors.ENDC}")
        print(f"     {Colors.CYAN}• 'means': {Colors.BOLD}Float vector{Colors.ENDC}{Colors.CYAN} (adata.var){Colors.ENDC}")
        print(f"     {Colors.CYAN}• 'variances': {Colors.BOLD}Float vector{Colors.ENDC}{Colors.CYAN} (adata.var){Colors.ENDC}")
        print(f"     {Colors.CYAN}• 'variances_norm': {Colors.BOLD}Float vector{Colors.ENDC}{Colors.CYAN} (adata.var){Colors.ENDC}")
...
def _postprocess_dispersions_seurat(
    ...
    if len(gen_indices) == 0:
        return
    print(f"   {EMOJI['warning']} {Colors.WARNING}Gene indices {Colors.BOLD}{gen_indices}{Colors.ENDC}{Colors.WARNING} fell into a single bin: normalized dispersion set to 1{Colors.ENDC}")
    print(f"   {Colors.WARNING}💡 Consider decreasing `n_bins` to avoid this effect{Colors.ENDC}")
...
def _subset_genes(
    ...
    *,
    ...
    if n_top_genes > adata.n_vars:
        print(f"   {EMOJI['info']} {Colors.BLUE}n_top_genes ({Colors.BOLD}{n_top_genes:,}{Colors.ENDC}{Colors.BLUE}) > n_vars ({Colors.BOLD}{adata.n_vars:,}{Colors.ENDC}{Colors.BLUE}), returning all genes{Colors.ENDC}")
        n_top_genes = adata.n_vars
    disp_cut_off = _nth_highest(dispersion_norm, n_top_genes)
    print(f"   {Colors.CYAN}📊 Top {Colors.BOLD}{n_top_genes:,}{Colors.ENDC}{Colors.CYAN} genes correspond to normalized dispersion cutoff: {Colors.BOLD}{disp_cut_off:.4f}{Colors.ENDC}")
...
def highly_variable_genes(  # noqa: PLR0913
    ...
    *,
    ...
    """
    print(f"\n{Colors.HEADER}{Colors.BOLD}{EMOJI['start']} Highly Variable Genes Selection:{Colors.ENDC}")
    print(f"   {Colors.CYAN}Method: {Colors.BOLD}{flavor}{Colors.ENDC}")
    if n_top_genes is not None:
        print(f"   {Colors.CYAN}Target genes: {Colors.BOLD}{n_top_genes:,}{Colors.ENDC}")
    if batch_key is not None:
    ...
    n_hvg = df['highly_variable'].sum()
    total_genes = len(df)
    print(f"\n{Colors.GREEN}{EMOJI['done']} HVG Selection Completed Successfully!{Colors.ENDC}")
    print(f"   {Colors.GREEN}✓ Selected: {Colors.BOLD}{n_hvg:,}{Colors.ENDC}{Colors.GREEN} highly variable genes out of {Colors.BOLD}{total_genes:,}{Colors.ENDC}{Colors.GREEN} total ({n_hvg/total_genes*100:.1f}%){Colors.ENDC}")
    ...
    print(f"     {Colors.CYAN}• 'highly_variable': {Colors.BOLD}Boolean vector{Colors.ENDC}{Colors.CYAN} (adata.var){Colors.ENDC}")
    print(f"     {Colors.CYAN}• 'means': {Colors.BOLD}Float vector{Colors.ENDC}{Colors.CYAN} (adata.var){Colors.ENDC}")
    print(f"     {Colors.CYAN}• 'dispersions': {Colors.BOLD}Float vector{Colors.ENDC}{Colors.CYAN} (adata.var){Colors.ENDC}")
    print(f"     {Colors.CYAN}• 'dispersions_norm': {Colors.BOLD}Float vector{Colors.ENDC}{Colors.CYAN} (adata.var){Colors.ENDC}")
